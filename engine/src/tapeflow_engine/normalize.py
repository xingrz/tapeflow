...
def _capture_ranges(s):
    ...
    real gaps. Falls back to the whole tc span when the engine supplies no coverage."""
...
def _hdv_capture(s, files_by_tag):
    # span the TC across all of this capture's sessions: if the record-run TC restarts mid-capture,
    # the last GOP's tc0/tc1 is a tiny restarted session near zero, which would otherwise collapse the
    # capture (and the tape) onto it — take the min/max over its coverage ranges (a no-op otherwise)
    cov = s.get("coverage") or []
    ...
    return {
        ...
        "axis": [s["shift"], s["shift"] + s["ngops"]],   # GOP units (axis-fallback only; TC mode uses tc)
        "tcSpan": [tc0, tc1],
...
def from_hdvmerge(hdv, working_dir, files_by_tag):
    ...
    tape overlap by TC. A tape whose record-run TC restarts mid-capture is handled by spanning each
    capture's tcSpan across all its sessions (see ``_hdv_capture``) so the TC axis doesn't collapse
    onto the tiny restarted tail."""
    fps = hdv["fps"]
    segs = hdv["segments"]
    sources = hdv["sources"]
    damage = _hdv_damage(hdv, fps)
    captures = [_hdv_capture(s, files_by_tag) for s in sources]
    # tape TC bounds from the captures' session-spanning tcSpans (not segs[-1].tc_end, which on a TC
    # reset is the tiny restarted tail) so the range — and the whole TC axis — doesn't collapse
    tc0s = [c["tcSpan"][0] for c in captures if c["tcSpan"][0]]
    tc1s = [c["tcSpan"][1] for c in captures if c["tcSpan"][1]]
    tape = {
        "tcStart": min(tc0s, key=_tc_key) if tc0s else (segs[0].get("tc") if segs else None),
        "tcEnd": max(tc1s, key=_tc_key) if tc1s else (segs[-1].get("tc_end") if segs else None),
